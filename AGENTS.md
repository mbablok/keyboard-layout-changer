...
- Logic under test is UI-free and injected (`KeyboardLayoutController` takes run/sleep/loadMapping closures, `LaunchAtLogin` takes `LoginItemControls`); keep SwiftUI/`Process`/`SMAppService` glue thin and untested.
- `~/.hidutil-swap-cmd-opt.json` is the only mapping source (`KeyMapping.load` throws when it cannot be read); there is deliberately no built-in copy to fall back to.
